...
function BottomNav({ userRole, currentPage, onNavigate }: BottomNavProps) {
  const supervisorTabs = [
    { id: "dashboard", label: "Overview", icon: Home, path: "/Staff/dashboard" },
    { id: "myKpi", label: "My KPI", icon: Target, path: "/Staff/myKpi" },
    { id: "account", label: "Account", icon: User, path: "/Staff/account" },
  ];

  const tabs = userRole === "Staff" ? supervisorTabs : [];
...
export function SalesBottomNav() {
  ...
  if (!user || user.role !== "Staff" || isRedirecting) return null;
