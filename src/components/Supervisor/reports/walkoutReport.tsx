...
export const SupervisorWalkoutReportPage: React.FC<WalkoutReportPageProps> = ({ 
    startDate, 
    endDate 
}) => {
                ...
                <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <p className="text-xs text-gray-500 font-mono mt-1">
                            ID: {staffId} &nbsp;|&nbsp; Item: {typeof itemName === 'string' ? itemName : itemName.name} &nbsp;|&nbsp; Type: {typeof type === 'string' ? type : type.name}
                        </p>
                    </div>
                </div>
    ...
    useEffect(() => {
        const fetchWalkoutReport = async () => {
            ...
            try {
                const res = await axiosInstance.get("/supervisor/walkoutReport", {
                    params: {
                        start: startDate,
                        end: endDate,
                    },
    ...
    const handleExportPDF = async () => {
        ...
        try {
            const { data } = await axiosInstance.get(
                `/supervisor/walkoutReport/export?format=pdf&start=${startDate}&end=${endDate}`,
                { responseType: 'blob' }
            );
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const { data } = await axiosInstance.get(`/supervisor/walkoutReport/export`, {
